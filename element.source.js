customElements.define("file-size", class extends HTMLElement {
    connectedCallback(
      ...
      source = this.getAttribute("src") || "https://file-size.github.io/element.js",
      img, // global state to load gzthermal IMG once
      ...
      gzthermal = () => { // load GZThermal IMG function:
        if (!img) {
          this.querySelector("i").innerHTML = "loading gzthermal";
          img = this.querySelector("img");
          img.onerror = (e) => new Error("rate limit");
          img.onload = (e) => {
      ...
      fetch(source, { mode: "cors" })
        .then((response) => {
          ...
          this.innerHTML =
            ...
            `<summary>` +
          `<b style="color:dark${this.length < maxbytes ? "green" : "red"};display:inline-block;width:4em;text-align:right">${
              this.length
            }</b> &nbsp;Bytes ${source} <i/>` +
            `</summary><img/>` +
            `</details>`;
